        ...
        if iter_count > 0 and short_term_img_pool_size > 0:
            ...
            for iter in tqdm(range(iter_count), disable=self.silence, desc="GSDF training"):    
                ...
                if dice_number < short_term_train_prob: # [ 0, 1 ], 0.5 then means 50 % prob.
                    ...
                    weight_down_rate = 4**(down_rate_long_term-down_rate_short_term) # was 4^x (bitwise XOR): replay views silently got 4x the intended depth-loss weight
                ...
                # ----------------
                # Object mask "cookie cutter": object pixels get NO supervision at all
                # (NOT the sky alpha->0 loss), so the background behind a removed object
                # stays reconstructable from the other views that see past it.
                # Multiplying after the sky block above ANDs the two keep-masks.
                non_obj_mask = None
                if viewpoint_cam.obj_mask_on:
                    cur_obj_mask = viewpoint_cam.obj_mask_list[train_down_rate]
                    non_obj_mask = ~cur_obj_mask

                    if rendered_normal is not None:
                        rendered_normal = rendered_normal * non_obj_mask
                    if depth_normal is not None:
                        depth_normal = depth_normal * non_obj_mask
                    if dist_distortion is not None:
                        dist_distortion = dist_distortion * non_obj_mask
                ...
                # sky: drop sky pixels from the RGB loss so bright sky can't bias colour/exposure
                if viewpoint_cam.sky_mask_on:
                    nsm = non_sky_mask[:, pixel_v_min:pixel_v_max, :].to(rendered_rgb_image_for_loss.dtype)
                    rendered_rgb_image_for_loss = rendered_rgb_image_for_loss * nsm
                    gt_rgb_image_for_loss = gt_rgb_image_for_loss * nsm

                # objects: drop masked-object pixels from the RGB loss entirely (cookie cutter),
                # zeroing BOTH rendered and gt so neither the object's colours nor the empty
                # render behind it generate any gradient there
                if non_obj_mask is not None:
                    nom = non_obj_mask[:, pixel_v_min:pixel_v_max, :].to(rendered_rgb_image_for_loss.dtype)
                    rendered_rgb_image_for_loss = rendered_rgb_image_for_loss * nom
                    gt_rgb_image_for_loss = gt_rgb_image_for_loss * nom
                ...
                if rendered_depth is not None and gt_depth_image is not None and self.config.lambda_depth > 0:
                    ...
                    if non_obj_mask is not None: # no depth supervision on masked-object pixels
                        valid_depth_mask = valid_depth_mask & non_obj_mask
